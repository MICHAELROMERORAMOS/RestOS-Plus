...
export default function ReportsPage() {
  const { state, formatMoney, operationalSummary } = useRestaurant()
  const closed = state.orders.filter((order) => order.status === 'closed')
  const completedCount = Number(operationalSummary?.completedOrdersToday || closed.length)
  const ticketAverage = completedCount ? state.sales / completedCount : 0
