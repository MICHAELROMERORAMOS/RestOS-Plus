...
function modeLabel(order, tableLabel) {
  if (order.mode === 'delivery') return `Domicilio Â· ${order.delivery?.customerName || 'Cliente'}`
  if (order.mode === 'quick') return `Servicio rÃ¡pido Â· Orden #${order.id}`
    ...
    <div className="modal open" onClick={onClose}>
      <div className="modal-card" onClick={(event) => event.stopPropagation()}>
        <div className="section-title">
          <div><h3>ð¦ Historial de entregas</h3><p className="muted">Solo productos entregados y no anulados.</p></div>
          <button className="btn" onClick={onClose}>Ã</button>
        </div>
        {!orders.length ? <div className="empty-block">No hay pedidos entregados todavÃ­a.</div> : (
          <div className="list">
            {orders.map((order) => {
              ...
              return <article className="card" key={order.id}>
                <div className="section-title">
                  <div><h4>{modeLabel(order, tableLabelFor?.(order))}</h4><small>Orden #{order.id}{order.invoiceNumber ? ` Â· Factura ${order.invoiceNumber}` : ''}</small></div>
                ...
                <div className="list">
                  {items.map((item) => {
                    ...
                    return <div className="list-row" key={`${order.id}-${item.lineId}`}><span>{quantity} Ã {item.name}<small>{formatMoney(price)} c/u</small></span><b>{formatMoney(price * quantity)}</b></div>
                  })}
                </div>
